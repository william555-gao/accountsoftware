(function ($) {
    var lastHeight = $('.cntl-state').height();
    var iconHeight = $('.cntl-icon').height();
    // alert(lastHeight);
    $.fn.cntl = function (options) {

        /* default settings */
        var settings = $.extend({
            revealbefore: 2000, /* this is the amount of "scroll padding" to allow (the more, the later the state will be revealed) */
            ...
            function revealElements() {
                ...
                for (var i = 0; i < states.length; i++) {
                    ...
                    if (states[i].top > windowtop && states[i].top < windowbtm) {
                        ...
                        bar_fill.height(tbf + lastHeight + iconHeight);
                    }
                }
            }
